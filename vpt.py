...
def distFunction(lis):
    pass

def sumListItems(num_entry, lis, idx):
...
def buyFrac(num_entry, lis, typ, cut_val=None):
    lsval = lis.copy()
    lstyp = typ.copy()
    newval=[]
...
def modelList(lsprice, lstype):
    ''' Model the null psteps with the neighbouring psteps'''
    zs, nzs = getListIndices(lsprice)
    # modelled price step
    nls = lsprice.copy()
    if len(nzs)==0: 
        pass
...
for i in dfile: 
    ...
    for date in td:
        ...
        Npsteps=modelList(psteps,types)
        ...
        while idx<len(valb)+1:
            vptbsum = sumListItems(num_entry, vptb, idx)
            vptb_sum.append(vptbsum)
            vbsum = sumListItems(num_entry, valb, idx)
            valb_sum.append(vbsum)
            pvstbsum=sumListItems(num_entry, nvstb, idx)
            pvstb_sum.append(pvstbsum)
            idx += 1
        idx = 1
        while idx<len(vals)+1:
            vptssum = sumListItems(num_entry, vpts, idx)
            vpts_sum.append(vptssum)
            vssum = sumListItems(num_entry, vals, idx)
            vals_sum.append(vssum)
            pvstssum=sumListItems(num_entry, nvsts, idx)
        ...
        buyfrac_val, bft_val = buyFrac(nentry, values , types, cut_val=50)
